...
int main()
{
	...
	PDU_data_DRB_2_t *A;
	A = (PDU_data_DRB_2_t*)malloc(N * sizeof(PDU_data_DRB_2_t)); //initialisation of temp. pointer for t-reordering purposes
	...
	while(l = recv(clientSocket, buffer, 11, 0)) //receiving packets
	{
		printf("\nloop time %.6f\n", current_time(start) - loop); // time duration of previous loop
		loop = current_time(start);// update loop to start of current loop
		
		PDU_data_DRB_2_t *p = PDU_data_DRB_2_t_ini(); //initialisation of temp. pointer to store data
...
*/
		PDU_data_DRB_2_t_oph(p); //remove header
		
		reord_drb2(A,p,dbuf,&discard_idx,start); //reordering
	}
