...
int main()
{
	...
	srand(time(0));
	int n = 15; //no. of PDU_DRB_1, sample

	uint16_t ibuf[] = {0,1,3,5,6,8,9,11,4,10,7,12,14,13,15}; //sample sequence no.s

	for (int i = 0; i < n; i++) //should be replaced with receive function
	{
		PDU_data_DRB_1_t *ob = PDU_data_DRB_1_t_ini(); //initialisation of pointer for transmission purposes
		// ob->R_SN += (1<<12) - 1 - i;
		ob->DC_R_SN |= ibuf[i]; // assign header - seq no. and R-bits
		printf("-----DRB_1-----\n");

		PDU_data_DRB_1_t_data(ob); //data generation, should be replaced with received data
		...
		printf("\n");
		PDU_data_DRB_1_t_oph(ob); //add header
		printf("\nHEADER ADDED!\n");
		printf("%d bytes data: ", ob->data_len);
